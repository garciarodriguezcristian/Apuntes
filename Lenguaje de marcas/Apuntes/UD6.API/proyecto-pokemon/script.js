...
function mostrarPokemon(datos) {
    ...
    contenedor.innerHTML = `
        ...
        <p><strong>Hablidades:</strong> ${datos.abilities.map(habilidad => habilidad.ability.name).join(", ")}</p>
        <p><strong>Stats:</strong> ${datos.stats.map(estadistica => `${estadistica.stat.name} : ${estadistica.base_stat}`).join(", ")}</p>`;
        
        // Muestra los stats de cada pokemon junto a su base_stats
