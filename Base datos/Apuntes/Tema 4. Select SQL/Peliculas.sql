...
select titulo from pelicula where duracion > 150 order by duracion;
...
select count(*), genero from pelicula group by genero;
...
select round(avg(duracion)) from pelicula;
...
select titulo from pelicula where id = (select id_pelicula from reparto group by id_pelicula having count(id_actor) = 2);

-- 20. Seleccionar las películas estrenadas en los últimos 10 años.
select titulo from pelicula where extract(year from now()) - anio < 10;
-- otra froma:
select titulo from pelicula where anio > extract(year from now()) - 10;

-- 22 dime la moda del año de nacimiento de los actores.
select anio_nacimiento as moda from actor
where anio_nacimiento > 1970
group by anio_nacimiento
having count(anio_nacimiento) = (select max(anio_repetido) from (
select count(anio_nacimiento) as anio_repetido 
from actor
where anio_nacimiento > 1970
group by anio_nacimiento)
as anio_moda);
