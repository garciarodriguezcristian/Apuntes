...
select ciudad, avg(edad) as promedioEdad from empleado group by ciudad;
...
select count(id) as total_ventas, sum(importe) as importe_total from venta;
...
select distinct nombre from empleado where departamento in (select nombre from departamento);
...
select cliente, importe from venta where importe > (select avg(importe) from venta);