...
select distinct barrio, nombre from procesion;
...
select nombre from procesion where dia_semana like 'Viernes%';

-- 7. Mostrar los puntos de paso de las procesiones que pasan por 'Catedral'.
select punto from itinerario where id_procesion = (select id_procesion from itinerario where punto = 'Catedral');
...
select count(*) as cantidad_procesiones from procesion;
...
select sum(numero_costaleros) from hermandad;
...
select count(*) as num_procesiones, barrio from procesion group by barrio;

-- 27. Calcular el peso total de los pasos agrupados por estilo.
select sum(peso) from paso group by estilo;
...
select count(*) from itinerario where hora_paso > '20:00';
...
select nombre from paso where not nombre like '%Virgen%';
...
select nombre from procesion where not nombre like 'La%';
...
select date_format(hora_paso, '%H') as hora_paso, punto, (select nombre from procesion where id = id_procesion) as procesion from itinerario;
...
select round(peso, -3), peso from paso;
