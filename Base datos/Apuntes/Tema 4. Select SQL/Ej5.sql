...
VALUES
('Construcción de hospitales', 2, '2023-01-01', '2026-12-31', 50000000.00),
('Reparación de carreteras', 3, '2023-04-01', '2026-09-30', 30000000.00),
('Capacitación docente', 1, '2023-06-01', '2026-11-30', 10000000.00);
...
select char_length(nombre), nombre from Ciudadano;
...
select round(presupuesto_asignado/10000) from Actividad;

-- 7. Calcular la diferencia entre los ingresos anuales de cada ciudadano y el promedio de ingresos.
select round(ingresos_anuales - (select avg(ingresos_anuales) from Ciudadano)) as diferencia_ingresos, nombre from Ciudadano;

-- 8. Obtener el entero más cercano hacia abajo y hacia arriba de los presupuestos de los ministerios.
select floor(presupuesto) as redonde_abajo, ceil(presupuesto) as redondeo_arriba from Ministerio;

-- 9. Generar un número aleatorio para asignar un identificador único temporal a cada actividad.
select descripcion, round(1+ (rand() * 10)) as temporal from Actividad;
...
SELECT DATE_FORMAT(fecha_inicio, '%d/%m/%Y') as inicio, DATE_FORMAT(fecha_inicio, '%d/%m/%Y') as fin from Actividad;
-- 12. Calcular cuántos días faltan para que termine cada actividad.
select datediff(fecha_fin, now()) as diasParaFinalizar, descripcion from Actividad;
-- 13. Extraer el mes y el año de las fechas de nacimiento de los ciudadanos.
select extract(month from  fecha_nacimiento) as mes,extract(year from fecha_nacimiento) as anio,concat(nombre," ",apellido) as nombre from Ciudadano;
