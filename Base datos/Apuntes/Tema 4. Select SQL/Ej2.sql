...
select * from equipos order by nombre_equipo;
-- 7. Filtrar jugadores que juegan como "Delantero" y tienen más de 20 goles.
select * from jugadores where posicion = "Delantero" && goles > 20;
-- 8. Obtener el promedio de goles por partido para un equipo con un ID específico.
select avg(goles_local + goles_visitante) as media_goles from partidos where id_equipo_local = 1 or id_equipo_visitante = 1;
-- 9. Mostrar partidos jugados en enero de 2023.
select * from partidos where fecha between "2023-01-01" and "2023-01-31" order by fecha;

-- 10. Contar jugadores de un equipo con ID específico.
select count(id_jugador) from jugadores where id_equipo = 1;
...
order by id_equipo_local asc, total_goles desc;
...
select id_equipo_local, count(id_equipo_local) as partidos_local from partidos group by id_equipo_local order by partidos_local desc;
