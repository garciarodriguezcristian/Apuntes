...
where jugadores.id_jugador is null;

-- 6. Jugadores por posición en cada equipo
select equipos.nombre,jugadores.posicion, count(*) as num_jugadores_por_posicion
from equipos
left join jugadores on equipos.id_equipo = jugadores.id_equipo
group by equipos.nombre, jugadores.posicion;

-- 7. Comparativa de presupuestos vs gasto en jugadores
select equipos.nombre as equipo,equipos.presupuesto_anual, 
sum(jugadores.salario_anual) as total_salarios,
(equipos.presupuesto_anual - sum(jugadores.salario_anual)) as diferencia
from equipos
join jugadores on equipos.id_equipo = jugadores.id_equipo
group by equipo;
