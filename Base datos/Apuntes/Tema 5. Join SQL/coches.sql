...
select personas.nombre, personas.apellido, coches.marca, coches.modelo 
from personas join coches on personas.id = coches.id_dueño;

-- 2. Coches con información de dueños como su nombre y su provincia
select coches.*, dueño.nombre, dueño.provincia 
from coches join personas as dueño on coches.id_dueño = dueño.id;

-- 3. Personas que tienen coches y su estado civil
select personas.nombre, personas.estado_civil 
from personas join coches on personas.id = coches.id_dueño;
...
select ocupacion
from personas join coches on personas.id = coches.id_dueño 
where floor(datediff(now(),fecha_nacimiento)) > 30 
group by personas.ocupacion;

-- Nivel 2: LEFT JOIN

-- 6. Todas las personas y sus coches (incluyendo sin coches)
select nombre, apellido, coches.* 
from personas left join coches on personas.id = coches.id_dueño order by id asc;

-- 7. Personas sin coches registrados
select personas.*
from personas left join coches on personas.id = coches.id_dueño
where id_dueño is null;

-- 8. Provincias con y sin representación de coches
select provincia, count(coches.id) as coches
from personas left join coches on personas.id = coches.id_dueño
group by provincia
order by coches desc;


-- 9. Nivel de estudios de las personas sin coche
select nivel_estudios, personas.nombre
from personas left join coches on personas.id = coches.id_dueño 
where id_dueño is null;
...
select * 
from personas left join coches on personas.id = coches.id_dueño
where ocupacion = "Jubilado" and id_dueño is not null;


-- Nivel 3: JOINS con Agregaciones

-- 11. Número de coches por persona
select count(*) as num_coches, concat(nombre, ' ', apellido) as completo 
from coches join personas on personas.id = coches.id_dueño 
group by completo order by num_coches desc;

-- 12. Marca más común entre personas de cada provincia
select coches.marca, provincia, count(*) 
from coches join personas on personas.id = coches.id_dueño 
group by marca,provincia order by provincia;

-- 13. Promedio de edad de dueños por marca de coche
select avg(floor(datediff(now(),fecha_nacimiento)/365)) as promedio, coches.marca as marca
from personas join coches on personas.id = coches.id_dueño group by coches.marca;

-- 14. Total de ingresos de dueños por color de coche
select sum(ingresos_anuales) as total_ingresos, coches.color
from personas join coches on personas.id = coches.id_dueño group by coches.color;

-- 15. Municipios con más coches
select municipio, count(*) as numero_coches
from personas join coches on personas.id = coches.id_dueño 
group by municipio 
having numero_coches = (select max(numero_coches) from 
...
SELECT personas.nombre, coches.*
FROM personas
JOIN coches ON coches.id_dueño = personas.id
WHERE personas.id IN (
  SELECT id FROM personas WHERE padre IS NOT NULL OR madre IS NOT NULL
  UNION
  SELECT padre FROM personas WHERE padre IS NOT NULL
  UNION
  SELECT madre FROM personas WHERE madre IS NOT NULL
);

-- 17. Parejas con algún hijo y sus coches
select madre as id_madre, padre as id_padre, coches.* 
from personas 
join coches on coches.id_dueño in (personas.madre, personas.padre)
where madre is not null && padre is not null;

-- 18. Personas de la misma familia con más de dos coches
select id as hijos, madre, padre 
from personas
...
group by personas.id,madre,padre
having num_coches > 2;
