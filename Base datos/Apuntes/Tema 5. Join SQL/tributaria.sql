...
select contribuyentes.nombre, declaraciones.*
from contribuyentes
...
select contribuyentes.*, declaraciones.*
from contribuyentes
right join declaraciones on contribuyentes.id_contribuyente = declaraciones.id_contribuyente;

-- 4. Contar declaraciones por contribuyente
select contribuyentes.id_contribuyente as id, contribuyentes.nombre as c, count(declaraciones.id_declaracion) as num_dec
from contribuyentes
left join declaraciones on contribuyentes.id_contribuyente = declaraciones.id_contribuyente
group by c,id;

-- 5. Obtener contribuyentes sin declaraciones
select contribuyentes.nombre
from contribuyentes
left join declaraciones on contribuyentes.id_contribuyente = declaraciones.id_contribuyente
where declaraciones.id_contribuyente is null;

-- 6. Diferencias a pagar o devolver por contribuyente
select contribuyentes.nombre as contri, sum(declaraciones.diferencia) as diferencia_total
from contribuyentes
join declaraciones on contribuyentes.id_contribuyente = declaraciones.id_contribuyente
group by contribuyentes.id_contribuyente;
